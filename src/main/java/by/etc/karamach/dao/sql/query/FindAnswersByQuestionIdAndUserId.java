...
public final class FindAnswersByQuestionIdAndUserId {
    ...
    public static final String STATEMENT = "select answers.id, answers.description, answers.isRight from answers inner join questions q on answers.questions_id = q.id where (answers.questions_id = ?) AND (q.owner_id = ?);";
